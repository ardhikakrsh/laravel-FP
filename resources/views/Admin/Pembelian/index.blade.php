...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <h1 class="bg-indigo-700 text-3xl font-semibold text-white dark:text-indigo-600 pl-4 pt-2 rounded-t-lg">Data Pembelian</h1>
    <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4 bg-indigo-700">
        ...
        <div class="flex justify-end w-full md:w-auto">
            <a href="{{ route('pembelian.create') }}" class="flex items-center px-4 py-2 text-sm font-medium text-white bg-black rounded-lg hover:bg-indigo-600 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
            ...
            <tr class="bg-white text-center border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-center">
                        <img src="{{ asset('assets/img/' . $pembelian->barang->foto) }}" alt="{{ $pembelian->barang->nama }}" width="125">
                    </div>
                </td>
                <td class="px-6 py-4">{{ $pembelian->user->nama }}</td>
                <td class="px-6 py-4">{{ $pembelian->barang->nama }}</td>
                <td class="px-6 py-4">{{ $pembelian->tanggal_pembelian }}</td>
                <td class="px-6 py-4">Rp{{ number_format($pembelian->total_harga, 0, ',', '.') }}</t</td>
                <td class="px-6 py-4">
                    <div class="flex space-x-2 justify-center">
                        <!-- View Button -->
                        <a href="{{ route('pembelian.show', $pembelian->id) }}" class="flex items-center justify-center px-2 py-1 text-blue-500 hover:text-blue-700 bg-blue-100 rounded-md transition duration-300 transform hover:scale-105">
                        ...
                        <a href="{{ route('pembelian.edit', $pembelian->id) }}" class="flex items-center justify-center px-2 py-1 text-yellow-500 hover:text-yellow-700 bg-yellow-100 rounded-md transition duration-300 transform hover:scale-105">
                        ...
                        <!-- Delete Button -->
                        <form action="{{ route('pembelian.destroy', $pembelian->id) }}" method="POST" onsubmit="return confirmDelete();" class="inline">
                            @csrf
                            @method('DELETE')
